...
**Heading-Base** (definiert in shared.css):
- `h1, h2`: DM Serif Display, weight 400, line-height 1.15, letter-spacing -0.02em, color var(--navy)
- `h3, h4`: DM Sans, weight 700, line-height 1.3, color var(--navy-mid)
- Individuelle Regeln überschreiben nur font-size, margin und ggf. color.

**Heading-Spacing-Tokens** (tokens.css):
- `--heading-gap-h2`: 16px — Abstand unter h2 vor Fliesstext
- `--heading-gap-h3`: 12px — Abstand unter h3 vor Fliesstext
- `--heading-gap-h4`: 8px — Abstand unter h4 vor Fliesstext
...
- `margin-bottom` bei Headings immer über `--heading-gap-*` Tokens, nie als Hardcode-Pixel.
...
| `shared.css` | Layout, Navigation, Footer, Karten, Buttons, Homepage-Sektionen, Heading-Base |
...
4. **Heading-Tokens?** `margin-bottom` bei h2/h3/h4 über `--heading-gap-*`, nicht als Pixel.
5. **Kontrast geprüft?** Jede neue Text-Hintergrund-Kombination gegen WCAG AA prüfen.
6. **Mobile getestet?** Breakpoints: 600px (Mobile), 768px (TOC), 900px (Nav), 1100px (TOC-Sidebar).
7. **Reduced Motion?** Neue Animationen hinter Media-Query.
